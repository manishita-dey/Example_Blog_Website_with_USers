...
class BlogPost(db.Model):
    ...
    # Create Foreign Key, "users.id" the user refers to the tablename of User.
...
def register():
    ...
    if form.validate_on_submit():
        hashed_pass = generate_password_hash(password=form.password.data, salt_length=8, method= 'pbkdf2:sha256')
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            ...
            return redirect(url_for('login'))
        new_comment = Comment(text =form.body.data, comment_author = current_user,
